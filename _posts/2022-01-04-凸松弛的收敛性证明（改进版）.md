...
和前述证明方式类似，我们引入一个微小扰动造成松弛约束从非严格不等式变成严格不等式，并判断目标函数的解是否下降。下面我们将根据目标后项中和$$\beta_i$$相关的项进行分类讨论。可以看到，这个两个相关的项分别由下式\eqref{eq:beta_eqns}中的$$g_1(\beta_i)$$和 $$g_2(\beta_i)$$来表示。定义戴维斯函数$$f_{d,i}=A+Bv_{ave,i}+C\beta_{i}$$，定义区间势能变化为$$U_i=Mg\Delta H_i$$。
