...
&=\eta_t^{'} \sum_i F_i \Delta d+ (\eta_b-\eta_t^{'})\sum_k\(0.5M(\beta_{k} -\beta_{k-1})+(A+B v_{ave,k}+C\beta_k)\Delta d+Mg\Delta H_k\)
\end{align}
$$
...
和前述证明方式类似，我们需要引入一个微小扰动造成松弛约束从非严格不等式编程严格不等式。下面我们将根据目标后项中和$$beta_i$$相关的项进行分类讨论。可以看到，这个两个相关的项分别由下式\eqref{eq:beta_eqns}中的$$g_1(\beta_2)$$和 $$g_2(\beta_i)$$来表示。
