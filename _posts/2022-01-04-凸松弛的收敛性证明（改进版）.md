...
\begin{align}
\label{eq:beta_eqns}
g_1(\beta_i)&=0.5M(\beta_i-\beta_{i-1})\\
&+(A+B v_{ave,i}+C\beta_i)\Delta d+ Mg\Delta H_i\nonumber\\
g_2(\beta_i)&=0.5M(\beta_{i+1}-\beta_i)\nonumber\\
&+(A+B v_{ave,i+1}+C\beta_{i+1})\Delta d+ Mg\Delta H_{i+1}\nonumber
\end{align}
$$
...
定义戴维斯函数$$f_{d,i}=A+Bv_{ave,i}+C\beta_{i}$$，定义区间势能变化为$$U_i=Mg\Delta H_i$$。 依据$$g_1(\beta_i)\leq 0$$,可以得到式\eqref{eq:beta_ul_1}：

$$
\begin{align}
\label{eq:beta_ul_1}
&0.5M(\beta_i-\beta_{i-1})+(f_{d,i})\Delta d+U_i \leq 0\\
&(0.5M+C\Delta d)\beta_i \leq 0.5\beta_{i-1}-(A+Bv_{ave,i})\Delta d-U_i\nonumber\\
&\beta_i \leq \frac{0.5\beta_{i-1}-(A+Bv_{ave,i})\Delta d-U_i}{0.5M+C\Delta d}\nonumber
\end{align}
$$
...
\begin{align}
\label{eq:beta_ul_2}
0.5M\beta_i &\leq 0.5M\beta_{i+1}+f_{d,i+1}\Delta d+U_{i+1}\nonumber\\
\beta_i &\leq \frac{0.5M\beta_{i+1}+(f_{d,i+1})\Delta d+U_{i+1}}{0.5M}\nonumber
\end{align}
$$
...
* 当式\eqref{eq:beta_ul_1}变为等式，目标函数中和$$\beta_i$$的相关目标后项，均为非负项，和$$\beta_i$$相关的项，仅存在于目标前项。前面已经论述，在此条件下$$beta_i$$的正扰动将带来目标函数的上升。
