...
和前述证明方式类似，我们引入一个微小扰动造成松弛约束从非严格不等式变成严格不等式，并判断目标函数的解是否下降。下面我们将根据目标后项中和$$\beta_i$$相关的项进行分类讨论。可以看到，这个两个相关的项分别由下式\eqref{eq:beta_eqns}中的$$g_1(\beta_2)$$和 $$g_2(\beta_i)$$来表示。定义戴维斯函数$$f_{d,i}=A+Bv_{ave,i}+C\beta_{i}$$，定义区间势能变化为$$U_i=Mg\Delta H_i$$。

$$
\begin{align}
\label{eq:beta_eqns}
g_1(\beta_i)&=0.5M(\beta_i-\beta_{i-1})+f_{d,i}\Delta d+ U_i\\
g_2(\beta_i)&=0.5M(\beta_{i+1}-\beta_i)+f_{i+1}\Delta d+ U_{i+1}\nonumber
\end{align}
$$

下面，我们分三类条件来讨论。注意当$$g_1(\beta_2)$$和 $$g_2(\beta_i)$$均为正的情况出现的时候，意味着没有目标后项需要考虑。在这种情况下，证明方法和在[文三](http://lushaofeng.github.io/blog/convex_3)相同，在这里不赘述。
