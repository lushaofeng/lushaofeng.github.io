...
通过以上两种情况的探讨，可以看到当引入一个正微小扰动$$\epsilon$$，使得原有的松弛不等式$$1\leq v_{ave,i} \alpha_i$$成为一个严格不等式的时候，目标函数的函数值均上升。该松弛约束的等式条件为目标函数最优的必要条件。问题得证。
...
可见，在条件一下，在$$\beta_i$$中引入微小增量，导致目标函数的增加。
...
式\eqref{eq:beta_case_2}的前面一项为正，而后面一项也为正，可见目标函数的增量为正。可见，在条件二下的微小的扰动将带来目标函数值的上升。
...
可见，当达到前两个上界的$$\beta_i$$，微小的正扰动将带来目标函数的上升。换言之，**条件三**最终会在边界上被转换为某一个最优解的必要条件，然后在这一条件下，可以看到松弛约束将会严格收敛在等号上。

## 条件三中的“奇点”情况
最后讨论当$$\beta_i=v_{lim,i}$$的情况。注意到当前$$\beta_i$$为常数，而原来的松弛约束$$v_i^2\leq \beta_i$$成为了限速的常规约束，换而言之，该松弛约束不复存在。因此在这个时候似乎不再需要证明是否有松弛约束收敛的问题，因为凸松弛约束在这个时候完全被另外一个“常规限速约束”替代而消失了。套用列车最优运控理论的术语，当列车接近限速时候，会出现最优控制策略的跃迁，这个时候属于控制策略的“奇点（Singularity）”。当出现$$\beta_i=v_{lim,i}$$的时候，不知道是否也能认为这个解对应了列车运行的奇点。这个时候之前讨论的所有扰动法都失效了。如果在这个时候用经典扰动法来证明，将无法得到速度会贴近限速，也即“v_i^2”和“\beta_i”无法证明必须贴合。
