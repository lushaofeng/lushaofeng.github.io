...
不过通过仔细研究发现，[文三](http://lushaofeng.github.io/blog/convex_3)的证明存在一个明显的瑕疵，该明显瑕疵来自于我们将列车在再生制动和牵引过程的电机效率等同于1，目标函数值由$$\sum_i F_i\Delta d_i$$来替代。但是当电机的转换效率不为1的时候，[文三](http://lushaofeng.github.io/blog/convex_3)中简化版的目标函数并不等同于原本的目标函数。因此[文三](http://lushaofeng.github.io/blog/convex_3)的证明只能在非常理想的条件，即电机转换效率为1下成立。为了解决这个问题，本文将着重探讨如何在更为通常的条件下来证明凸松弛条件最终会收敛于等号——即证明优化模型中的两组凸松弛约束为绝对松弛（Relaxation is exact）。

为了方便陈述，我们做如下定义。
* **正负边界能量：** 由能量守恒方程式$$F_i\Delta d=0.5M(\beta_i-\beta_{i-1})+f_i\Delta d+Mg\Delta H_i$$决定的正负能量值。理论上，边界能量的的大小由能量守恒定律和电机特性共同决定。为了简化说明，我们假定能量守恒方程式包含了电机特性，也即电机的牵引力和再生制动力能够满足所有运行条件，不需要额外借助空气制动等机械制动。 当边界能量为正，电机做正功；当边界能量为负，电机做负功。
...
* **目标函数：** 目标函数为$$\sum_i(E_i^{+} \eta_t^{'}+ E_i^{-} \eta_b)$$。其中$$\eta_t^{'}=1/\eta_t\geq 1$$为列车牵引效率的倒数，$$0\geq \eta_b\geq 0$$为列车的再生制动效率。
* **编号：**$$j\in \mathbb{K}$$和$$k\in \mathbb{J}$$分别用来表示正负边界能量集合中的非零项的编号，有$$\mathbb{K}\cup \mathbb{J}= \mathbb{I}$$
...
基于以上定义，我们给出如式\eqref{eq:obj_fun}展示的目标函数的具体表达。将边界负能量拆分为两项，分别为$$\eta_t^{'}E_i^-$$和$$(\eta_b-\eta_t^{'})E_i^-$$。前项与边界负能量进行合并，人为构建前述$$F_i\Delta d$$的求和。如[文三](http://lushaofeng.github.io/blog/convex_3)所示，该求和可以表示为：$$\sum_i F_i\Delta d_i\\=0.5M(v_t^2-v_0^2)+\sum_i f_i\Delta d+Mg\Delta H\\=B\sum_i v_{ave,i}\Delta d+ C \sum_i \beta_i\Delta d+Mg\Delta H+0.5M(v_t^2-v_0^2) +NA$$。
...
和前述证明方式类似，我们引入一个微小扰动造成松弛约束从非严格不等式变成严格不等式，并判断目标函数的解是否下降。下面我们将根据目标后项中和$$beta_i$$相关的项进行分类讨论。可以看到，这个两个相关的项分别由下式\eqref{eq:beta_eqns}中的$$g_1(\beta_2)$$和 $$g_2(\beta_i)$$来表示。
...
\end{align}
$$
下面，我们分三类条件来讨论。注意当$$g_1(\beta_2)$$和 $$g_2(\beta_i)$$均为正的情况出现的时候，意味着没有目标后项需要考虑。这种情况下，证明方法和在[文三](http://lushaofeng.github.io/blog/convex_3)相同，在这里不赘述。
...
\Delta C\epsilon \Delta d\eta_t^{'} - (\eta_b-\eta_t^{'})(0.5 M\epsilon )>0\\
\end{align}
$$
...
这个时候可以发现，由于戴维斯常数C相对于列车质量M较小，$$\eta_b C\Delta d<0.5(\eta_t^{'}-\eta_b)M$$。因此，引入一个正的扰动，将导致目标函数的增加。我们需要注意到，此时的$$\beta_i$$并没有在最优解的必要条件上，因此需要增加$$\beta_i$$进一步带来目标函数的下降，直至达到$$\beta_i$$的边界。

那么，我们可以考虑$$\beta_i$$在到达边界条件之后$$\beta_i$$的情况会如何。根据**条件三**的定义，可分别算出以下两个关系式。
...
套用列车最优运控理论的术语，当列车接近限速时候，会出现最优控制策略的跃迁，这个时候属于控制策略的“奇点（Singularity）”。当出现$$\beta_i=v_{lim,i}$$的时候，不知道是否也能认为这个解对应了列车运行的奇点。这个时候之前讨论的所有扰动法都失效了。如果在这个时候用经典扰动法来证明，将无法得到速度会贴近限速，也即“v_i^2”和“\beta_i”无法证明必须贴合。
